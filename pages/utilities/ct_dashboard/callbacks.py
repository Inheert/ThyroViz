...
You can find the following group of callback:
    ...
    - Bar chart to analyse ct_studies repartition from different sources
    - Tab with multiple chart to analyse ct_studies by date  
...
"""
CardStatUpdate():
    This callback is used to update the evolution rate of ct_studies by year.
...
CardStat2Update():
    This callback is used to update the ct_studies repartition by ct_sponsors class.
    The first Input is the ct_sponsors class selection and the second is the selected category.
...
def TextUpdate(data):
    if data in all_category:
        return data, "Sub-category repartition", "Sub-category repartition by ct_studies type"
    else:
        return "Overview", "Category repartition", "Category repartition by ct_studies type"
